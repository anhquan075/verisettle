  ...
  return (
    <section aria-live="polite" aria-label="Wallet and testnet readiness" className={`rounded-2xl border ${ready ? "border-cyan-200/18 bg-cyan-300/[0.045]" : "border-white/10 bg-white/[0.025]"} p-4`}>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {!wallet.address && wallet.wallets.map((candidate) => <Button key={candidate.id} type="button" size="sm" onClick={() => void wallet.connect(candidate.id)} disabled={wallet.busy} className={`veri-action font-semibold ${wallet.selectedWallet?.id === candidate.id ? "bg-cyan-300 text-slate-950 hover:bg-cyan-200" : "border border-white/10 bg-white/[0.04] text-slate-100 hover:bg-white/[0.08]"}`}>{wallet.busy && wallet.selectedWallet?.id === candidate.id ? <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" /> : <WalletCards className="mr-2 h-3.5 w-3.5" />}Connect {candidate.name}</Button>)}
