  ...
  return (
    <section aria-live="polite" aria-label="Wallet and testnet readiness" className={`rounded-2xl border ${ready ? "border-cyan-200/18 bg-cyan-300/[0.045]" : "border-white/10 bg-white/[0.025]"} p-4`}>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {wallet.address ? <ConnectButton.Custom>{({ mounted, openAccountModal }) => <Button type="button" size="sm" variant="outline" onClick={openAccountModal} disabled={!mounted || wallet.busy} className="veri-action border-cyan-100/20 text-cyan-50 hover:bg-cyan-300/10"><WalletCards className="mr-2 h-3.5 w-3.5" />Change wallet</Button>}</ConnectButton.Custom> : null}
