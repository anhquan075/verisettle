    ...
    <section id="judge-route" tabIndex={-1} className="scroll-mt-24 overflow-hidden rounded-[1.75rem] border border-cyan-200/15 bg-[#071216] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300" aria-labelledby="judge-route-heading">
      ...
      <div className="grid gap-5 p-5 sm:p-7 lg:grid-cols-[1fr_0.78fr]">
        ...
        <aside className="rounded-2xl border border-teal-200/15 bg-teal-300/[0.055] p-5">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-300">Release needs a valid receipt, source, terms, parties, and unused query.</p>
          <p className="mt-3 flex items-start gap-2 text-xs leading-5 text-teal-50"><CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0" />The same proof cannot release twice.</p>
        </aside>
      </div>
      ...
      <div className="border-t border-fuchsia-200/15 bg-[linear-gradient(115deg,rgba(71,19,89,0.24),rgba(7,18,22,0.96)_58%)] p-5 sm:p-7">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex max-w-2xl items-start gap-3">
            ...
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-fuchsia-100/80">V3 governed recovery</p>
              <h3 className="mt-2 text-lg font-semibold text-white">Disputes need 2 of 3 approvals.</h3>
              <p className="mt-2 text-sm leading-6 text-slate-300">V3 separates dispute authority from custody. One signer cannot release or refund.</p>
            </div>
          </div>
