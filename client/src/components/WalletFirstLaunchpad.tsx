      ...
      <div className="relative grid gap-6 xl:grid-cols-[1.15fr_0.85fr] xl:items-start">
        <div>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {!wallet.address ? <ConnectButton.Custom>{({ mounted, openConnectModal }) => <Button onClick={openConnectModal} disabled={!mounted} className="veri-action bg-cyan-300 font-semibold text-slate-950 hover:bg-cyan-200"><WalletCards className="mr-2 h-4 w-4" />Choose wallet</Button>}</ConnectButton.Custom> : <><Button onClick={() => void signInWithWallet()} disabled={wallet.busy} className="veri-action bg-cyan-300 font-semibold text-slate-950 hover:bg-cyan-200"><ShieldCheck className="mr-2 h-4 w-4" />{wallet.busy ? "Signing…" : "Sign wallet session"}</Button><Button type="button" variant="outline" onClick={wallet.changeWallet} disabled={wallet.busy} className="veri-action border-cyan-100/20 bg-black/10 text-cyan-50 hover:bg-cyan-300/10"><WalletCards className="mr-2 h-4 w-4" />Change wallet</Button></>}
