    ...
    <div className="mx-auto w-full max-w-7xl space-y-8 pb-12">
      ...
      <section className="veri-protocol-hero relative overflow-hidden rounded-[2rem] border border-cyan-100/15 p-6 sm:p-9">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-end">
          <div>
            ...
            <p className="mt-5 max-w-xl text-sm leading-7 text-slate-300 sm:text-base">Sepolia source. Attestcoin proof. CC3 settlement.</p>
          ...
          <ol className="veri-protocol-route" aria-label="Cross-chain protocol path">
            <li><span className="veri-route-marker"><span>01</span><Network className="h-4 w-4" /></span><div><p>Source</p><strong>Ethereum Sepolia</strong><span>Buyer receipt binds the terms.</span><a href={`${TESTNET_NETWORKS.sepolia.explorerUrl}/address/${VERISETTLE_CONTRACTS.source}`} target="_blank" rel="noreferrer">Inspect emitter <ArrowUpRight className="h-3.5 w-3.5" /></a></div></li>
            <li><span className="veri-route-marker"><span>02</span><ShieldCheck className="h-4 w-4" /></span><div><p>Verify</p><strong>Attestcoin ASC</strong><span>Receipt, terms, one-time use.</span><a href="https://docs.creditcoin.org/attestcoin-protocol/dapp-builder-infrastructure/attestcoin-sdk-usc-sdk.md" target="_blank" rel="noreferrer">Read proof reference <ArrowUpRight className="h-3.5 w-3.5" /></a></div></li>
            <li><span className="veri-route-marker"><span>03</span><Waypoints className="h-4 w-4" /></span><div><p>Settle</p><strong>Creditcoin CC3</strong><span>Proof unlocks native tCTC.</span><a href={`${TESTNET_NETWORKS.creditcoin.explorerUrl}/address/${VERISETTLE_CONTRACTS.escrowAsc}`} target="_blank" rel="noreferrer">Inspect ASC <ArrowUpRight className="h-3.5 w-3.5" /></a></div></li>
          </ol>
        </div>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <section className="veri-reference-surface rounded-[1.75rem] border border-white/10 p-5 sm:p-7">
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-white/10 pb-5"><div><p className="veri-kicker">Deployment index</p><h2 className="mt-2 font-veri-display text-2xl font-semibold tracking-[-0.055em] text-white">Inspect the contracts.</h2></div><FileCode2 className="h-6 w-6 text-cyan-200" /></div>
