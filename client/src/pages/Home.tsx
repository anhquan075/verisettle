...
import { ArrowUpRight, Github, LockKeyhole, Network, ShieldCheck } from "lucide-react";
...
const routeBeats = [
  ["01 · Source", "Ethereum Sepolia", "Buyer acceptance receipt"],
  ["02 · Verify", "Attestcoin ASC", "Receipt + terms + one-time proof"],
  ["03 · Settle", "Creditcoin CC3", "tCTC release"],
] as const;
    ...
    <motion.div initial={false} animate={isLeavingForJudge ? { opacity: 0, y: -10 } : { opacity: 1, y: 0 }} transition={{ duration: shouldReduceMotion ? 0 : 0.16, ease: [0.23, 1, 0.32, 1] }} className="veri-shell min-h-screen overflow-x-clip text-white">
      ...
      <header className="veri-site-header sticky top-0 z-40 mx-auto flex w-full max-w-7xl items-center justify-between gap-3 border-b border-white/[0.06] bg-[#061014]/92 px-5 sm:px-8">
        ...
        <div className="flex min-w-0 shrink-0 items-center gap-2">
          <nav aria-label="Landing navigation" className="hidden items-center gap-1 text-sm text-slate-300 lg:flex">
            <a href="#execution" className="rounded-md px-2.5 py-1.5 transition-colors hover:bg-white/[0.06] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">How it works</a>
            <a href="#evidence" className="rounded-md px-2.5 py-1.5 transition-colors hover:bg-white/[0.06] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">Proof record</a>
            <a href="#security" className="rounded-md px-2.5 py-1.5 transition-colors hover:bg-white/[0.06] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">Boundary</a>
            <button onClick={openJudgeRoute} disabled={isLeavingForJudge} className="rounded-md px-2.5 py-1.5 transition-colors hover:bg-white/[0.06] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 disabled:cursor-wait disabled:opacity-60">Judge route</button>
          </nav>
          <ConnectionQualityIndicator />
          <Button onClick={() => setLocation("/app")} className="veri-action h-9 shrink-0 bg-white px-3 text-[#07161a] hover:bg-cyan-50">Open workspace <ArrowUpRight className="ml-1.5 h-4 w-4" /></Button>
        </div>
      </header>
      ...
      <main id="landing-content" className="relative mx-auto max-w-7xl px-5 pb-20 pt-8 sm:px-8 lg:pt-10">
        <section className="grid gap-10 lg:grid-cols-[1.08fr_0.92fr] lg:items-center lg:gap-14">
          <div>
            ...
            <motion.div initial={entrance} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: shouldReduceMotion ? 0 : 0.14, ease: [0.23, 1, 0.32, 1] }} className="mt-7 flex flex-col gap-3 sm:flex-row">
              ...
              <Button size="lg" variant="outline" onClick={() => setLocation("/app")} className="veri-action border-white/15 bg-white/[0.025] text-white hover:bg-white/[0.08]">
          ...
          <motion.div initial={shouldReduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: shouldReduceMotion ? 0 : 0.1, ease: [0.23, 1, 0.32, 1] }} className="relative mx-auto w-full max-w-xl">
            <div className="veri-proof-canvas">
              ...
              <div className="veri-proof-canvas__top">
                <span>Live testnet route</span>
                <span className="inline-flex items-center gap-1.5 text-teal-100">
                  <span className="veri-receipt-pulse h-2 w-2 rounded-full bg-teal-300" /> Receipt-bound
                </span>
              </div>
              ...
              <p className="mt-4 max-w-md text-sm leading-6 text-slate-300">Receipts move it. Promises do not.</p>
            </div>
          </motion.div>
        </section>
          ...
          <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-start lg:gap-14">
            <div className="veri-action-poster">
              <h2 className="font-veri-display text-4xl font-semibold leading-[0.96] tracking-[-0.065em] text-white">
                One order.<br />
                <span className="text-teal-200">Three receipts.</span>
              </h2>
              <p className="mt-5 text-sm leading-7 text-cyan-50/80">Each step is public.</p>
              <Button onClick={openJudgeRoute} disabled={isLeavingForJudge} variant="outline" className="veri-action mt-7 border-cyan-200/25 text-cyan-100 hover:bg-cyan-300/10 disabled:cursor-wait disabled:opacity-60">
                Judge evidence <ArrowUpRight className="ml-2 h-4 w-4" />
              </Button>
            </div>
        ...
        <motion.section id="security" initial={cardEntrance} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.42, ease: [0.23, 1, 0.32, 1] }} className="veri-boundary-statement mt-20 grid gap-8 rounded-[1.75rem] border border-teal-200/10 p-6 sm:p-10 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <div className="space-y-4 text-sm leading-7 text-slate-300">
            <p>Release checks receipt success, source event, terms, and one-time use.</p>
            <p>Testnet receipts are real. Physical delivery is outside this protocol.</p>
            <a href="https://github.com/gluwa/usc-testnet-bridge-examples" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 font-semibold text-cyan-100 hover:text-cyan-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">
      ...
      <footer className="relative border-t border-white/8 bg-black/10 px-5 py-8">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 text-center text-xs text-slate-500 sm:flex-row sm:text-left">
          ...
          <p>
            Public testnet · No real-fund custody · Acceptance receipt only ·
