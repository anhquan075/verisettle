import { Accessibility, ArrowLeft, ArrowUpRight, Check, CheckCircle2, Copy, ShieldCheck } from "lucide-react";
    ...
    <div className="veri-shell min-h-screen overflow-x-clip text-white">
      ...
      <main id="judge-evidence" className="relative mx-auto max-w-6xl px-5 pb-24 pt-12 sm:px-8 sm:pt-18">
        <section className="mx-auto max-w-3xl text-center">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-base leading-7 text-slate-300">Attestcoin verifies the acceptance receipt, NOT physical delivery. Featured receipts use distinct buyer and seller wallets. V3 recovery is separate and 2-of-3.</p>
          <div id="judge-actions" className="mt-7 flex flex-wrap justify-center gap-3"><Button onClick={() => document.getElementById("judge-route")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="veri-action bg-cyan-300 font-semibold text-[#06191f] hover:bg-cyan-200">See proof <ArrowUpRight className="ml-2 h-4 w-4" /></Button><Button onClick={() => setLocation("/protocol")} variant="outline" className="veri-action border-white/15 bg-white/[0.025] text-white hover:bg-white/[0.08]">Contracts <ShieldCheck className="ml-2 h-4 w-4" /></Button><Button onClick={() => setLocation("/contrast-audit")} variant="outline" className="veri-action border-cyan-200/20 bg-cyan-200/[0.04] text-cyan-50 hover:bg-cyan-200/[0.1]">Contrast audit <Accessibility className="ml-2 h-4 w-4" /></Button></div>
        </section>
      ...
      <footer className="relative border-t border-white/8 bg-black/10 px-5 py-8"><div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-3 text-center text-xs text-slate-500 sm:flex-row sm:text-left"><p>© 2026 VeriSettle. All rights reserved.</p><p className="flex items-center gap-3">Public testnet evidence · No real-fund custody · No physical-delivery verification <button onClick={() => setLocation("/contrast-audit")} className="veri-action font-semibold text-cyan-100 hover:text-cyan-50">Contrast audit <ArrowUpRight className="ml-1 inline h-3 w-3" /></button></p></div></footer>
    </div>
  );
}
