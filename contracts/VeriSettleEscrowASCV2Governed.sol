...
/**
 * @title VeriSettleEscrowASCV2Governed
 * @notice V2-policy escrow successor whose dispute path is controlled only by a
 * threshold governance contract, never by a single owner key.
 * @dev It is a new deployment because V2's deployed `onlyOwner` authority is immutable.
 */
contract VeriSettleEscrowASCV2Governed is ReentrancyGuard {
    ...
    error InvalidDisputeGovernance();
    error InvalidSeller();
    ...
    constructor(
        ...
        if (disputeGovernance_ == address(0)) revert InvalidDisputeGovernance();
