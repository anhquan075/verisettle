...
- [ ] Audit tracked source and documentation for Manus-specific branding or project markers, removing only safe non-functional references needed for external repository review.
- [ ] Rewrite the root README in concise plain language for hackathon evaluators, including local setup, test commands, real testnet contracts, security boundaries, and reviewer evidence links.
- [ ] Make `anhquan075/verisettle` public after the review cleanup is validated, without publishing secrets or changing deployed application behavior.
