...
- [ ] Preserve verified wallet-session and funding receipt state across return visits, and embed the completed CC3 and Sepolia explorer receipts in the authenticated workspace.
- [ ] Restore completed funding receipt data from the verified SIWE identity before a returning browser wallet reconnects, so both embedded receipt cards appear immediately after sign-in.
