...
- [x] Complete the DoraHacks GitHub login and hacker registration, then prepare the project submission for user review without posting it. Quan Nguyen was registered as a solo hacker and the verified RWA submission draft was prepared with public evidence.
- [x] Submit the DoraHacks project only after the user explicitly confirms the final form contents and submission action. The user explicitly approved the terms and final action; DoraHacks confirmed VeriSettle was submitted and is under organizer review.
...
- [x] Rebuild and publish the walkthrough as a native 1920 × 1080 Full HD video using true-width real-interface captures, recalibrated scroll positions, restrained callouts, approved narration, and white outlined subtitles. The verified native Full HD version is publicly uploaded and linked in the submission materials.
- [x] Correct the submitted demo’s screenshot framing so every real-interface capture fully fits the widescreen video composition without narrow crops or letterboxing, then republish the verified replacement. The verified 1920 × 1080 wide-fit video is public, linked in the repository materials, and prominent in the submitted DoraHacks description; the original sidebar resource cannot be edited after review submission.
- [x] Audit every walkthrough callout bounding box against its real interface target, correcting any offset, overreach, or obstruction before the next publication. The judge proof-route and wallet SIWE/readiness overreach were replaced with tighter cursor-led targeting; verification frames confirm alignment on the real public controls.
- [x] Rebuild the walkthrough as a truthful interaction-led evaluator flow with a restrained cursor highlight and visible clicks on public routes and controls, without simulating wallet signatures, transactions, or external explorer activity. The verified Full HD version replays real public Judge proof, See proof, Contracts, Workspace, and Wallet checks controls; it does not open a provider, request a signature, or send a transaction.
- [x] Verify that the public Vercel deployment is built from the latest `main` commit, then safely refresh and re-check it if deployment metadata is stale. Vercel reported a successful production deployment for `9e8bc1329fee4296dc6475ed4c610546d5c75759`, which matches local and GitHub `main`; `https://verisettle-testnet.vercel.app` returned HTTP 200 from Vercel. No refresh was needed.
- [x] Remove the tracked public `docs/` directory and add ignore protection so future documentation and competitor-comparison files are not committed to the public repository. The public `main` branch no longer exposes `docs/`; its 58 tracked documentation/evidence files were removed, working-tree `docs/` is absent, and future files beneath that path are ignored.
- [x] Audit tracked source and documentation for Manus-specific branding or project markers, removing only safe non-functional references needed for external repository review. Unused template/debug artifacts and an unreferenced branded dialog were removed; public assets now use a neutral route, while functional platform integrations remain intact.
- [x] Rewrite the root README in concise plain language for hackathon evaluators, including local setup, test commands, real testnet contracts, security boundaries, and reviewer evidence links.
- [x] Make `anhquan075/verisettle` public after the review cleanup is validated, without publishing secrets or changing deployed application behavior. Repository visibility was verified as `PUBLIC` on GitHub.
- [x] Prepare the user-provided DoraHacks profile information for form entry, then request the missing role before any registration or submission action. Quan Nguyen was successfully registered as a solo hacker with the supplied profile information; the later user-approved VeriSettle BUIDL submission is now under organizer review.
- [x] Populate and review the DoraHacks VeriSettle BUIDL draft with verified public evidence, then obtain explicit user approval before the irreversible final submission. The user approved the final action; VeriSettle was submitted to BUIDL CTC 2026 Fall in the RWA track and is now under organizer review.
