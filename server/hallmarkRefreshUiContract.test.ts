...
describe("evidence-studio and protocol-reference UI contract", () => {
  ...
  const ambientPresets = source("../client/src/lib/ambientPresets.ts");
  ...
  it("keeps the shared visual system responsive and reduced-motion safe", () => {
    ...
    expect(layout).toContain("const ambientPreset = ambientPresetForPath(location)");
    expect(layout).toContain('ProofFieldBackground variant={ambientPreset}');
    expect(proofField).toContain("useReducedMotion");
    expect(proofField).toContain("AMBIENT_PRESETS");
    expect(proofField).toContain("shouldReduceMotion ? { opacity: 0.42, x: \"0%\", y: \"0%\", scale: 1 }");
    expect(orbit).toContain("AMBIENT_PRESETS");
    expect(ambientPresets).toContain('"audit"');
    expect(ambientPresets).toContain('"protocol"');
    expect(ambientPresets).toContain("ambientPresetForPath");
    expect(routeLight).toContain("ambientPresetForPath");
    expect(styles).toContain(".veri-proof-field--audit");
    expect(styles).toContain(".veri-proof-field--protocol");
    expect(styles).toContain(".veri-route-light--protocol");
    expect(styles).toContain("pointer-events: none");
    expect(proofField).toContain("proof-field ambience");
    expect(styles).toContain("overflow-x: clip");
  });

  it("keeps route feedback and system contrast safeguards without visible appearance controls", () => {
    ...
    expect(app).toContain('path={"/contrast-audit"}');
    ...
    expect(home).not.toContain("BackgroundIntensityMenu");
    expect(home).not.toContain("HighContrastToggle");
    expect(publicJudgeEvidence).not.toContain("BackgroundIntensityMenu");
    expect(publicJudgeEvidence).not.toContain("HighContrastToggle");
    expect(layout).not.toContain("BackgroundIntensityMenu");
    expect(layout).not.toContain("HighContrastToggle");
    ...
    expect(highContrast).toContain("prefers-contrast: more");
